...
* **Используемые библиотеки:** Шаблоны стилей через Tailwind, компоненты `Button` из shadcn/ui.

### LoginModal

* **Назначение:** Модальное окно для ввода логина без пароля (авторизация через NextAuth Credentials).
* **Пропсы:** Нет явных пропсов, но открытие/закрытие контролируется глобальным состоянием (Zustand) или контекстом.
* **Взаимодействие:** Содержит поле ввода для логина (например, компонент `Input` из shadcn/ui) и кнопку "Войти". При отправке формы вызывает `signIn()` из NextAuth (через хук `useAuth` или напрямую) для создания сессии. После успешного логина автоматически закрывается. Показывает ошибки при невалидном логине.
* **Используемые библиотеки:** shadcn/ui (`Modal`, `Input`, `Button`), Framer Motion для плавной анимации появления/скрытия, Zustand для состояния видимости, `useAuth` для выполнения авторизации.
...
* **Используемые библиотеки:** shadcn/ui (`Input`), Tailwind для стилей, Zustand для хранения `searchQuery`.
...
* **Используемые библиотеки:** shadcn/ui (`Button`), Tailwind для стилизации.
...
* **Используемые библиотеки:** Tailwind для макета карточки, shadcn/ui для типографики и кнопок внутри карточки, Framer Motion для анимации появления (fade-in, scale).
...
* **Используемые библиотеки:** Tailwind для стиля бейджа, shadcn/ui возможен для Badge-стиля, Framer Motion — опционально для эффекта наведения.
...
* **Используемые библиотеки:** React Query (через `useLocations`), Zustand (для фильтров), shadcn/ui для кнопок и форм, Tailwind/Framer Motion для стилей и анимаций.
...
* **Взаимодействие:** При получении параметра `id` вызывает `useLocation(id)` `(useQuery(['location', id]))` для загрузки объекта; данные передаёт в `LocationDetail`. Кнопки `EditButton` и `DeleteButton` используют хуки `useUpdateLocation()` и `useDeleteLocation()` соответственно.
* **Используемые библиотеки:** React Query (`useQuery` для получения одной локации), Tailwind для разметки, shadcn/ui (`Button`), Framer Motion (необязательно для плавного появления контента).
...
* **Используемые библиотеки:** Tailwind (верстка), shadcn/ui (возможные компоненты `Card`, `Image`), Framer Motion для плавного анимационного эффекта при появлении страницы.
...
* **Используемые библиотеки:** shadcn/ui (`Button`), Tailwind.
...
* **Используемые библиотеки:** shadcn/ui (`Button`), Tailwind, React Query для мутации.
...
* **Используемые библиотеки:** React Hook Form (или аналог) для обработки формы, shadcn/ui для элементов формы, Tailwind для верстки.
...
* **Используемые библиотеки:** React Hook Form (взаимодействие с полями), shadcn/ui (`Form`, `Input`, `Textarea`, `Select`, `Button`, `File Upload`), Tailwind. Framer Motion может анимировать динамический добавление полей (например, новые теги).
...
> **Библиотеки для UI и анимаций:** Все компоненты стилизуются с помощью Tailwind CSS. Для унифицированного дизайна используются готовые элементы из библиотеки shadcn/ui (Buttons, Inputs, Cards, Modal, Form). Анимации при появлении элементов (загрузка, переходы, появление карточек) реализуются через Framer Motion (например, `motion.div` для плавного фейда или анимирования Skeleton-эффекта).
