...
**Актаульный код Header:** *Ещё не рализован*
...
* **Взаимодействие:** Содержит поле ввода для логина (например, компонент `Input` из shadcn) и кнопку "Войти". При отправке формы вызывает `signIn()` из NextAuth (через хук `useAuth` или напрямую) для создания сессии. После успешного логина автоматически закрывается. Показывает ошибки при невалидном логине.
* **Используемые библиотеки:** shadcn (`Modal`, `Input`, `Button`), Framer Motion для плавной анимации появления/скрытия, Zustand для состояния видимости, `useAuth` для выполнения авторизации.
**Актаульный код LoginModal:** *Ещё не рализован*
...
**Актаульный код AddLocationButton:** *Ещё не рализован*
...
**Актаульный код LocationList:** *Ещё не рализован*
...
**Актаульный код LocationCard:** *Ещё не рализован*
...
**Актаульный код SkeletonCard:** *Ещё не рализован*
...
**Актаульный код LocationListPage:** *Ещё не рализован*
